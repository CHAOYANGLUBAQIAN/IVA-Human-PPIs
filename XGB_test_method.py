...
results = pd.DataFrame(columns=['Model', 'Accuracy', 'Precision', 'Recall', 'F1','MCC','Specificity'])
##使用循环对所有模型验证
#对于每个折叠
for model_name, model in models.items():
    ...
    results = results.append({'Model': model_name,
                              ...
                              'MCC': np.mean(mcc_scores),
                                'Specificity':np.mean(specificitys)},ignore_index=True)
    # 输出指标的平均值和标准差
    ...
    print("Specificity: {:.2f} (+/- {:.2f})".format(np.mean(specificitys), np.std(specificitys)))